...
import org.tests.webdriver.AuthenticatedHtmlUnitDriver;
...
System.out.println("ccccromme??"+browserName);
		if (CHROME.equals(browserName)) {
			capability = DesiredCapabilities.chrome();
			System.out.println("inside chromw");
			setChromeDriver();
			webDriver = new ChromeDriver();
		                                 } 
		else if (FIREFOX.equals(browserName)) {
			System.out.println("inside webdriverfactory , inside if of firefox)");
			capability = DesiredCapabilities.firefox();
			
			System.out.println("capability for firefox");
			
			FirefoxProfile ffProfile = new FirefoxProfile();
			
			System.out.println("ffprofile created");

			// Authenication Hack for Firefox
			        if (username != null && password != null) {
				System.out.println("inside not null if of user n pswd");
				ffProfile.setPreference("network.http.phishy-userpass-length",
						255);
				capability.setCapability(FirefoxDriver.PROFILE, ffProfile);
			                                                   }
			System.out.println("outside not null if of user n pswd");
			capability.setCapability(CapabilityType.TAKES_SCREENSHOT, true);
			System.out.println("end of firefox if");
		                                            }
...
System.out.println("am i here");
		...
		/*try { System.out.println("inside try for remote webdriver");
		...
		} catch (MalformedURLException e) { System.out.println("inside catch malformed");
			e.printStackTrace();
		}*/
System.out.println("outside try remote");
		return webDriver;
	}
	...
	public static WebDriver getInstance(String browser, String username,
			String password) {
System.out.println("inside getinstance 2222");
		WebDriver webDriver = null;
